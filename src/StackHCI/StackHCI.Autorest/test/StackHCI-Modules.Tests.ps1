Describe 'StackHCI Module Dependency Functions' {
    ...
    Describe 'Import-DependentModule' {

        It 'Does not throw when module is already loaded with sufficient version' {
            Mock Get-Module {
                return [PSCustomObject]@{ Version = [System.Version]'3.0.0' }
            }
        ...
        It 'Throws when module is not available' {
            Mock Get-Module { return $null }
            Mock Remove-Module {}
            Mock Import-Module { throw 'Module not found' }
    ...
    Describe 'Check-DependentModules' {

        It 'Does not throw when all modules present' {
            Mock Get-Module {
                return [PSCustomObject]@{ Version = [System.Version]'99.0.0' }
            }
            { Check-DependentModules } | Should -Not -Throw
        ...
        It 'Throws with missing module names when modules are absent' {
            Mock Get-Module { return $null }
            Mock Remove-Module {}
            Mock Import-Module { throw 'not found' }
            { Check-DependentModules } | Should -Throw
